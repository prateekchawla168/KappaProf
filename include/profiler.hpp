...
public:
    struct Event
    {
        ...
        {
            long double multiplexingCorrection = static_cast<double>(data.timeEnabled - prev.timeEnabled) / (data.timeElapsed - prev.timeElapsed);
            if ((data.timeElapsed - prev.timeElapsed) == 0)
                multiplexingCorrection = 1.0;
            return static_cast<long double>(data.value - prev.value) * multiplexingCorrection;
        }
    };
    ...
    long double GetIPC()
    {
        return (long double)GetCounter("instructions") / (long double)GetCounter("cycles");
    }

    long double GetCPUs()
    {
        return (long double)GetCounter("task-clock") / (long double)(GetDuration() * 1e9);
    }

    long double GetAvgGHz()
    {
        return (long double)GetCounter("cycles") / (long double)GetCounter("task-clock");
    }
