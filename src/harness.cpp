...
void BenchmarkResult::DumpCSV()
{
    ...
    if (is_empty)
    {
        ...
        auto separator = ",";
        for (const auto &name_el : name_list)
            names_formatted += name_el + std::string(separator);
        names_formatted.pop_back(); // remove last comma

        csvfile << "Test_type,M,N,K,flops,wall-time," << names_formatted << "\n";
    }
    ...
    for (auto run : std::views::iota(warmupRuns, warmupRuns + runs))
    {
        ...
        for (auto &monitor : reports[run])
            counter_list.emplace_back(monitor.GetCount());
        // format into a csv line
        std::string counts_formatted = "";
        auto separator = ",";
        for (auto &count : counter_list)
            counts_formatted += std::to_string(count) + std::string(separator);
        counts_formatted.pop_back(); // remove last comma

        /* First counter is hw cycles by default */
        auto line = std::format("{},{},{},{},{},{},{}\n", name, m, n, k, flops, timeDurations[run], counts_formatted);
...
void BenchmarkResult::PrintReport()
{
