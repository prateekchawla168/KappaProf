...
void PerfEvent::RegisterCounter(const std::string &name, uint64_t type, uint64_t eventID, EventDomain domain = ALL)
{
...
void PerfEvent::PrintReport()
{
    ...
    {
        std::cout << std::format("{} : {}", names[i], GetCounter(names[i])) << std::endl;
    }
    return;
}
...
PerfEvent::PerfEvent()
{
    ...
    {
        auto &event = events[i];
        event.fd = static_cast<int>(syscall(SYS_perf_event_open, &event.pe, 0, -1, -1, 0));
        if (event.fd < 0)
        {
            std::stringstream errmsg;
